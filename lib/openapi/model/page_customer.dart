...
class PageCustomer {
  ...
  @override
  String toString() => 'PageCustomer[totalPages=$totalPages, totalElements=$totalElements, first=$first, last=$last, size=$size, content=$content, number=$number, sort=$sort, numberOfElements=$numberOfElements, pageable=$pageable, empty=$empty]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PageCustomer[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PageCustomer[$key]" has a null value in JSON.');
        });
        return true;
      }());
      ...
      return PageCustomer(
        ...
        content: Customer.listFromJson(json[r'content']) ?? const [],
  ...
  static Map<String, List<PageCustomer>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PageCustomer.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
